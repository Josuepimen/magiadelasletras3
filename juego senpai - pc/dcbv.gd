...
@onready var collision3 = $CollisionShape2D

@onready var music = $pronunciacionestante
...
func _input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton and event.pressed:
		if not alreadyClicked:
			...
			music.play()
			sprite.visible = true 
			collision3.disabled = true # Hacer el Sprite2D visible cuando se presiona el objeto
