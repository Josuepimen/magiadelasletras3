...
func _input_event(viewport, event, shape_idx):
	if not clicked and event is InputEventMouseButton and event.pressed:
		...
		animation_player.play("scale_up4")
		await(get_tree().create_timer(6.0)).timeout
		animated_sprite_2d_2.visible = true
		
		# Temporizador para hacer visible sprite_2d_3 después de 1 segundo
		await(get_tree().create_timer(1.0)).timeout
		animated_sprite_2d_3.visible = true
		
		# Temporizador para hacer visible sprite_2d_4 después de 1.3 segundos
